...
export default function Experience({ color }) {
  ...
  return (<>
    <Container maxW={isMobile ? '3xl' : '4xl'} id='education'>
      <Stack
        as={Box}
        textAlign={'center'}
          ...
          {education
            .map((exp, index) => (<Fade bottom>
              <Card key={exp.company} size='sm' id={`education-${index}`}>
                ...
                {showClassesIndex !== index && (<ButtonGroup variant='outline' pl={4} justifyContent={'center'}>
                  <Button
                  ...
                  <CardBody>
                    <Flex>
                      <List align='left' spacing={3}>
                        {exp.listItems.map((item, index) => (<ListItem key={index} pl={6}>
                          <Flex wrap='nowrap'>
                            <ListIcon
                              boxSize={6}
                              as={ChevronRightIcon}
                        ...
                        <Card key={classname.name}>
                          <Stack>
                            <CardBody align='left' h={[null, '40vh']}>
                              ...
                              <HStack flexWrap='wrap' pt={4} spacing={2}>
                                {classname.badges.map((badge) => (<Badge
                                  my={2}
                                  key={badge.text}
                                  colorScheme={badge.colorScheme}
                                >
